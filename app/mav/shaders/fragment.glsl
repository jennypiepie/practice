...
void main() {
    ...
    vec2 fakeUV = vec2(diff, dot(vec3(-1.,0,1.),vNormal));
    // fakeUV = abs(fakeUV);
    fakeUV = fract(fakeUV + vec2(time/20.,time/10.));

    float phi = acos(vNormal.y);
    float angle = atan(vNormal.x, vNormal.z);
    vec2 newFakeUV = vec2((angle+PI)/(2.*PI), phi/PI);

    vec4 txt = texture2D(uTexture, newFakeUV+ 0.2* cnoise(vec4(fakeUV*5., time/20., 0.), vec4(10.)));

    // gl_FragColor = vec4(fakeUV, 0., 1.);
    gl_FragColor = vec4(mix(vec3(1.), txt.rgb,1.),1.);
